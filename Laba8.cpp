...
public:
    ...
    std::string getAddress() const { // Геттер для address
        return address;
    }
    ...
    void setAddress(const std::string& newAddress) { // Сеттер для address
        if (!newAddress.empty()) {
            address = newAddress;
        }
        else {
    ...
    void displayInfo() const {
        std::cout << "Name: " << name << ", Age: " << age << ", Email: " << email << ", Address: " << address << std::endl; // Обновлено для вывода address
    }
};
...
int main() {
    ...
    person.setName("Nikita Shevchenko");
    person.setAge(19);
    person.setEmail("nikitasevcenko694@gmail.com");
    person.setAddress("Mikhail Nagibin Avenue, 5"); // Устанавливаем адрес
    ...
    person.setAddress(""); //Ошибка: Address cannot be empty
